...
def call_model(request: CallModelRequest):
    ...
    """
    log_event("Developer", f"Received LLM invocation request for model: {request.model}")
    try:
...
def compose_identity(request: ComposeIdentityRequest):
    ...
    """
    log_event("Provider", f"Identity composition request received for Agent ID: {request.agent_id}")
    try:
        ...
        log_event("Provider", f"Failed to compose Agent ID JWT: {str(e)}")
